...
import at.fabianachammer.pdusend.core.type.factory.parser.AtomicDataUnitFactoryCreator;
import at.fabianachammer.pdusend.core.type.factory.parser.DataUnitFactoryCreator;
...
class AtomicDataUnitFactoryCreatorTest {
	...
	@Test
	void createDataUnitFactoryWithMapContainingPredefinedValuesAsByteArraysReturnsAtomicDataUnitFactoryWithThisByteArraysAsPredefindevValues(){
		DataUnitFactoryCreator creator = new AtomicDataUnitFactoryCreator()
		def mapWithValuesAsByteArrays = [sizeInBits: 16, values:[first: [0, 1] as byte[]]]
		AtomicDataUnitFactory expected = AtomicDataUnitFactory.makeFromDataUnitSizeInBits(16)
		expected.addPredefinedValueByKey("first", [0, 1] as byte[])

		AtomicDataUnitFactory actual = creator.createDataUnitFactory(mapWithValuesAsByteArrays)

		assertEquals(expected, actual)
	}
	
	@Test
	void createDataUnitFactoryWithMapContainingPredefinedValuesAsListWithSizeInBitsGreaterThanRepresentedByPredefinedValuesReturnsAtomicDataUnitFactoryWithPredefinedValuesWhichHaveLeadingZeros(){
		DataUnitFactoryCreator creator = new AtomicDataUnitFactoryCreator()
		def mapWithValuesLowerThanMaxSpecifiedBySizeInBits = [sizeInBits: 9, values: [first: [1]]]
		AtomicDataUnitFactory expected = AtomicDataUnitFactory.makeFromDataUnitSizeInBits(9)
		expected.addPredefinedValueByKey("first", [0, 1] as byte[])
		
		AtomicDataUnitFactory actual = creator.createDataUnitFactory(mapWithValuesLowerThanMaxSpecifiedBySizeInBits)
